      ...
      jQuery.getJSON("/products/" + product_handle + ".js", function (product) {
        $(product.variants).each(function (i, v) {
          if (v.title == selectedOptions) {
            ...
            var original_price = parseFloat(v.compare_at_price / 100).toFixed(
              2
            );
        ...
        $(document).on("click", ".qv-add-button", function () {
          ...
          function processCart() {
            jQuery
              ...
              .done(function (cartItem) {
                console.log(cartItem.quantity)
                $(".header-actions-list .header-cart-link > span").html(cartItem.quantity);
                $(".header-actions-list .header-cart-link > span").addClass('active');
                $(".qv-add-to-cart-response")
                  .addClass("success")
                  .html(
                    "<span>" +
                      $(".qv-product-title").text() +
                      ' has been added to your cart. <a href="/cart">Click here to view your cart.</a>'
                  );
              })
              .fail(function ($xhr) {
